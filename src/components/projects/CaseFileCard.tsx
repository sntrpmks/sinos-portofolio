...
export function CaseFileCard({ project }: CaseFileCardProps) {
  ...
  return (
    <div className="card-minimal card-minimal-interactive p-6 flex flex-col justify-between gap-6 group">
      {/* Large Image Area */}
      <div className="w-full h-52 sm:h-60 rounded-xl bg-[#F0F0ED] border border-[#E6E6E3] overflow-hidden relative flex items-center justify-center">
        {project.coverImage && !imgError ? (
          ...
          <div className="flex flex-col items-center gap-2 text-[#8A8A8A]">
            ...
            <span className="text-xs font-mono-code font-bold">{project.caseFileId}</span>
          </div>
        )}
      </div>
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex flex-col gap-1">
          ...
          <span className="text-xs text-[#666666] font-medium">{project.role}</span>
        </div>
