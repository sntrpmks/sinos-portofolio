  ...
  return (
    <div className="pt-28 pb-20 px-4 sm:px-8 max-w-4xl mx-auto flex flex-col gap-10">
      ...
      >
        ...
        <span>
          {locale === "id" ? "KEMBALI KE Halaman Project" : "BACK TO PROJECTS"}
        </span>
      </Link>
      ...
      <SectionReveal className="card-minimal p-8 flex flex-col gap-6">
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-[#E6E6E3] pb-4">
          <div className="flex items-center gap-3 font-mono-code text-xs">
            <span className="px-2.5 py-0.5 rounded bg-[#E6F9F5] text-[#00695C] font-bold border border-[#B2F3E5]">
              {activeProj.caseFileId}
            </span>
            <span className="text-[#666666] font-medium">
              {locale === "id" ? "STATUS: SELESAI" : `STATUS: ${activeProj.status.toUpperCase()}`}
            </span>
          </div>
        ...
        <div className="flex flex-col gap-1">
          <h1 className="text-3xl sm:text-4xl font-bold text-[#171717]">{activeProj.title}</h1>
          <p className="text-xs font-mono-code text-[#00695C] font-semibold">
            {locale === "id" ? "Peran" : "Role"}: {activeProj.role} • {locale === "id" ? "Tahun" : "Year"}: {activeProj.year}
          </p>
        </div>
      ...
      {activeProj.gallery && activeProj.gallery.length > 0 && (
        <SectionReveal>
          <ProjectGallery gallery={activeProj.gallery} projectTitle={activeProj.title} />
        </SectionReveal>
      )}

      {/* Problem & Solution Grid */}
      <SectionReveal className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="card-minimal p-6 flex flex-col gap-3">
          ...
          <p className="text-sm text-[#2A2A2A] leading-relaxed">{activeProj.problem}</p>
        </div>
        ...
        <div className="card-minimal p-6 flex flex-col gap-3">
          <div className="flex items-center gap-2 text-[#171717]">
            ...
            <h3 className="text-base font-bold">
              {locale === "id" ? "Solusi & Pendekatan" : "The Engineering Solution"}
            </h3>
          </div>
          <p className="text-sm text-[#2A2A2A] leading-relaxed">{activeProj.solution}</p>
        </div>
      </SectionReveal>

      {/* Architecture Topology */}
      {activeProj.architecture && activeProj.architecture.length > 0 && (
        <SectionReveal>
          <ArchitectureDiagram nodes={activeProj.architecture} />
        </SectionReveal>
      )}

      {/* Implementation Highlights */}
      {activeProj.implementation && activeProj.implementation.length > 0 && (
        <SectionReveal className="card-minimal p-6 flex flex-col gap-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {activeProj.implementation.map((step, idx) => (
      ...
      <SectionReveal className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="card-minimal p-6 flex flex-col gap-3">
            <div className="flex items-center gap-2 border-b border-[#E6E6E3] pb-3">
              ...
              <h3 className="text-base font-bold text-[#171717]">
                {locale === "id" ? "Hasil & Dampak" : "Measurable Results"}
              </h3>
            </div>
        ...
        {activeProj.lessons && activeProj.lessons.length > 0 && (
          <div className="card-minimal p-6 flex flex-col gap-3">
            <div className="flex items-center gap-2 border-b border-[#E6E6E3] pb-3">
              <Lightbulb className="w-5 h-5 text-[#171717]" />
              <h3 className="text-base font-bold text-[#171717]">
                {locale === "id" ? "Pelajaran & Catatan" : "Key Lessons Learned"}
              </h3>
            </div>
            <ul className="flex flex-col gap-2 font-normal text-xs text-[#2A2A2A]">
              {activeProj.lessons.map((les, i) => (
                <li key={i} className="flex items-start gap-2">
